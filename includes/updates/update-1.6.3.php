...
function erp_acct_alter_invoice_receipts_1_6_3() {
    ...
    if ( ! in_array( 'transaction_charge', $cols ) ) {
        $wpdb->query(
            "ALTER TABLE {$wpdb->get_blog_prefix()}erp_acct_invoice_receipts ADD `transaction_charge` decimal(20,2) DEFAULT 0 AFTER `amount`;"
        );
    }
}
...
function erp_acct_insert_to_erp_acct_ledgers_1_6_3() {
    ...
    $check_data = $wpdb->get_row(
        $wpdb->prepare(
            "SELECT * FROM {$wpdb->get_blog_prefix()}erp_acct_ledgers WHERE slug = %s", [ 'bank_transaction_charge' ]
        )
    );
